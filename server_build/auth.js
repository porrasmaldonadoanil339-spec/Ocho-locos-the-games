...
router.post("/google", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const gUser = yield resp.json();
        const users = loadUsers();
        let user = users.find(u => u.googleId === gUser.sub);
        if (!user) {
            let username = ((_a = gUser.name) === null || _a === void 0 ? void 0 : _a.replace(/[^a-zA-Z0-9_]/g, "").substring(0, 18)) || "Player";
            let base = username;
            let counter = 1;
...
router.post("/facebook", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const appTokenResp = yield fetch(`https://graph.facebook.com/oauth/access_token?client_id=${appId}&client_secret=${appSecret}&grant_type=client_credentials`);
        const appTokenData = yield appTokenResp.json();
        const verifyResp = yield fetch(`https://graph.facebook.com/debug_token?input_token=${accessToken}&access_token=${appTokenData.access_token}`);
        const verifyData = yield verifyResp.json();
        if (!((_a = verifyData.data) === null || _a === void 0 ? void 0 : _a.is_valid)) {
            return res.status(401).json({ error: "invalid Facebook token" });
        }
        const meResp = yield fetch(`https://graph.facebook.com/me?fields=id,name&access_token=${accessToken}`);
        const fbUser = yield meResp.json();
        const users = loadUsers();
        let user = users.find(u => u.facebookId === fbUser.id);
        if (!user) {
            let username = ((_b = fbUser.name) === null || _b === void 0 ? void 0 : _b.replace(/[^a-zA-Z0-9_]/g, "").substring(0, 18)) || "Player";
            let base = username;
            let counter = 1;
            while (users.find(u => u.username.toLowerCase() === username.toLowerCase())) {
                username = `${base}${counter++}`;
...
router.get("/google/callback", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const userResp = yield fetch("https://www.googleapis.com/oauth2/v3/userinfo", { headers: { Authorization: `Bearer ${tokenData.access_token}` } });
        const gUser = yield userResp.json();
        const users = loadUsers();
        let user = users.find(u => u.googleId === gUser.sub);
        if (!user) {
            let username = ((_a = gUser.name) === null || _a === void 0 ? void 0 : _a.replace(/[^a-zA-Z0-9_]/g, "").substring(0, 18)) || "Player";
...
router.get("/facebook/callback", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    const { code, error: oauthError } = req.query;
    const appId = process.env.FACEBOOK_APP_ID || process.env.EXPO_PUBLIC_FACEBOOK_APP_ID;
    const appSecret = process.env.FACEBOOK_APP_SECRET;
    const redirectUri = `${req.protocol}://${req.get("host")}/api/auth/facebook/callback`;
    if (oauthError || !code)
        return sendOAuthResult(res, null, "Facebook auth cancelled");
    if (!appId || !appSecret)
        return sendOAuthResult(res, null, "Facebook not configured");
    try {
        const tokenResp = yield fetch(`https://graph.facebook.com/v18.0/oauth/access_token?client_id=${appId}&client_secret=${appSecret}&redirect_uri=${encodeURIComponent(redirectUri)}&code=${code}`);
        const tokenData = yield tokenResp.json();
        if (!tokenData.access_token)
            return sendOAuthResult(res, null, "No access token");
        const meResp = yield fetch(`https://graph.facebook.com/me?fields=id,name&access_token=${tokenData.access_token}`);
        const fbUser = yield meResp.json();
        const users = loadUsers();
        let user = users.find(u => u.facebookId === fbUser.id);
        if (!user) {
            let username = ((_a = fbUser.name) === null || _a === void 0 ? void 0 : _a.replace(/[^a-zA-Z0-9_]/g, "").substring(0, 18)) || "Player";
            let base = username;
            let counter = 1;
            while (users.find(u => u.username.toLowerCase() === username.toLowerCase())) {
                username = `${base}${counter++}`;
...
function sendOAuthResult(res, token, error) {
...
router.post("/profile", (req, res) => {
    ...
    const entry = { userId: payload.userId, data, updatedAt: new Date().toISOString() };
    if (idx >= 0)
        profiles[idx] = entry;
    else
        profiles.push(entry);
