...
exports.setupRooms = setupRooms;
const socket_io_1 = require("socket.io");
const multiplayerEngine_1 = require("../lib/multiplayerEngine");
const rooms = new Map();
...
function makeBotPlayer(playerIndex, seed) {
...
function queueKey(mode, playerCount) {
...
function publicState(room) {
    ...
    return {
        discardTop: (0, multiplayerEngine_1.multiGetTopCard)(gs),
...
function buildPlayersInfo(room) {
    return room.players.map(p => {
        ...
        return ({
            ...
            rankIcon: p.rankIcon,
            rankName: p.rankName,
            isBot: (_a = p.isBot) !== null && _a !== void 0 ? _a : false,
        });
    });
}
...
function setupRooms(httpServer) {
    ...
    io.on("connection", (socket) => {
        ...
        let matchmakingQueueKey = null;
        // ─── Room creation ────────────────────────────────────────────────────
        socket.on("create_room", ({ playerName, maxPlayers, mode = "classic", avatarColor, avatarIcon, level, rankColor, rankIcon, rankName, }) => {
            cleanOldRooms();
            ...
            const player = {
                ...
                playerIndex: 0,
                avatarColor: avatarColor !== null && avatarColor !== void 0 ? avatarColor : DEFAULT_PROFILE.avatarColor,
                avatarIcon: avatarIcon !== null && avatarIcon !== void 0 ? avatarIcon : DEFAULT_PROFILE.avatarIcon,
                level: level !== null && level !== void 0 ? level : DEFAULT_PROFILE.level,
                rankColor: rankColor !== null && rankColor !== void 0 ? rankColor : DEFAULT_PROFILE.rankColor,
                rankIcon: rankIcon !== null && rankIcon !== void 0 ? rankIcon : DEFAULT_PROFILE.rankIcon,
                rankName: rankName !== null && rankName !== void 0 ? rankName : DEFAULT_PROFILE.rankName,
                isBot: false,
            };
            const room = {
                code,
        ...
        // ─── Join by room code ─────────────────────────────────────────────────
        socket.on("join_room", ({ code, playerName, avatarColor, avatarIcon, level, rankColor, rankIcon, rankName, }) => {
        ...
        // ─── Matchmaking queue ─────────────────────────────────────────────────
        socket.on("join_matchmaking", ({ playerName, mode = "classic", playerCount = 2, avatarColor, avatarIcon, level, rankColor, rankIcon, rankName, }) => {
            var _a;
            if (inMatchmaking)
                return;
            ...
            (_a = io.sockets.sockets.get(socket.id)) === null || _a === void 0 ? void 0 : _a.emit("matchmaking_status", { queueSize: queue.length, needed: playerCount });
            if (queue.length >= playerCount) {
                ...
                const room = {
                    ...
                    players: matched.map((m, i) => (Object.assign(Object.assign({}, m), { playerIndex: i, isBot: false }))),
                ...
                for (const p of room.players) {
                    ...
                    if (s) {
                        s.join(code);
                        s._currentRoom = code;
                        s._myPlayerIndex = p.playerIndex;
                        s._inMatchmaking = false;
                        s._matchmakingQueueKey = null;
        ...
        socket.on("start_game", ({ botFill } = {}) => {
            const roomCode = currentRoom !== null && currentRoom !== void 0 ? currentRoom : socket._currentRoom;
            if (!roomCode)
                return;
        ...
        socket.on("play_card", ({ card }) => {
            var _a;
            const roomCode = currentRoom !== null && currentRoom !== void 0 ? currentRoom : socket._currentRoom;
            const pidx = myPlayerIndex >= 0 ? myPlayerIndex : ((_a = socket._myPlayerIndex) !== null && _a !== void 0 ? _a : -1);
            if (!roomCode || pidx < 0)
                return;
            const room = rooms.get(roomCode);
            if (!(room === null || room === void 0 ? void 0 : room.gameState))
                return;
            if (room.gameState.currentPlayerIndex !== pidx)
                return;
            if (room.gameState.phase === "game_over")
                return;
            try {
                const newState = (0, multiplayerEngine_1.multiPlayCard)(room.gameState, card);
        ...
        socket.on("draw_card", () => {
            ...
            if (room.gameState.phase === "game_over")
                return;
            try {
                const newState = (0, multiplayerEngine_1.multiDraw)(room.gameState);
                room.gameState = newState;
                broadcastGameState(room, io);
        ...
        socket.on("choose_suit", ({ suit }) => {
            ...
            if (room.gameState.phase !== "choosing_suit")
                return;
            try {
                const newState = (0, multiplayerEngine_1.multiChooseSuit)(room.gameState, suit);
                room.gameState = newState;
                broadcastGameState(room, io);
                scheduleAutoplay(room, io);
            }
        ...
        function leaveMatchmaking() {
            if (!inMatchmaking || !matchmakingQueueKey)
                return;
        ...
        function handleLeave() {
            ...
            const otherRealPlayers = room.players.filter(p => p.socketId !== socket.id && !p.isBot);
...
function startPreMatch(room, io) {
...
function startGame(room, io) {
    ...
    const gs = (0, multiplayerEngine_1.initMultiGame)(names, 8);
    room.gameState = gs;
    room.hands = [...gs.hands];
...
function broadcastGameState(room, io) {
    ...
    for (const player of realPlayers(room)) {
        const hand = (_a = room.gameState.hands[player.playerIndex]) !== null && _a !== void 0 ? _a : [];
        io.to(player.socketId).emit("game_state", Object.assign(Object.assign({}, pub), { myHand: hand, myPlayerIndex: player.playerIndex }));
    }
    if (room.gameState.phase === "game_over") {
        room.status = "done";
...
function scheduleAutoplay(room, io) {
    if (!room.gameState)
        return;
    if (room.gameState.phase === "game_over")
        return;
    ...
    const timer = setTimeout(() => {
        ...
        if (room.gameState.currentPlayerIndex === curr) {
            try {
                const newState = (0, multiplayerEngine_1.cpuPlayMulti)(room.gameState);
                room.gameState = newState;
                broadcastGameState(room, io);
