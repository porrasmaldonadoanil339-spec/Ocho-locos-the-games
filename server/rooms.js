...
import { initMultiGame, multiPlayCard, multiDraw, multiChooseSuit, multiGetTopCard, cpuPlayMulti, } from "../lib/multiplayerEngine";
const rooms = new Map();
...
export function setupRooms(httpServer) {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("start_game", ({ botFill } = {}) => {
            const roomCode = currentRoom ?? socket._currentRoom;
            if (!roomCode)
                return;
        ...
        socket.on("play_card", ({ card }) => {
            const roomCode = currentRoom ?? socket._currentRoom;
            const pidx = myPlayerIndex >= 0 ? myPlayerIndex : (socket._myPlayerIndex ?? -1);
            if (!roomCode || pidx < 0)
                return;
            const room = rooms.get(roomCode);
            if (!room?.gameState)
